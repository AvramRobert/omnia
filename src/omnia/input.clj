...
(defn process [seeker event]
  (case (:action event)
    :expand (expand seeker)
    :select-all (select-all seeker)
    :paste (deselect (paste seeker))
    :copy (deselect (copy seeker))
    :cut (deselect (cut seeker))
    :up (deselect (climb seeker))
    :down (deselect (fall seeker))
    :left (deselect (regress seeker))
    :right (deselect (advance seeker))
    :jump-left (deselect (jump seeker regress))
    :jump-right (deselect (jump seeker advance))
    :select-up (select (climb seeker))
    :select-down (select (fall seeker))
    :select-left (select (regress seeker))
    :select-right (select (advance seeker))
    :jump-select-left (select (jump seeker regress))
    :jump-select-right (select (jump seeker advance))
    :backspace (deselect (delete seeker))
    :delete (deselect (munch seeker))
    :enter (deselect (break seeker))
    :char (deselect (insert seeker (:key event)))
    seeker))
